...
(function (i9graph, undefined) {
    ...
    i9graph.animation_playing = false;
    i9graph.toggleAnimation = function(){  i9graph.animation_playing = !i9graph.animation_playing; redrawAll(); }

    i9graph.animation_playing_in_loop = false;
    i9graph.toggleAnimationLoop = function(){  i9graph.animation_playing_in_loop = !i9graph.animation_playing_in_loop; }



    function timeChanged(){
        if( i9graph.animationTime > i9graph.i9g.end ){
            if(i9graph.animation_playing_in_loop) i9graph.animationTime -= i9graph.i9g.end - i9graph.i9g.start;
            else                                  i9graph.animationTime = i9graph.i9g.end;
        }
        if( i9graph.animationTime < i9graph.i9g.start ){
            if(i9graph.animation_playing_in_loop) i9graph.animationTime += i9graph.i9g.end - i9graph.i9g.start;
        ...
        i9graph.computeMeasure('centrality','age');
        i9graph.computeMeasure('centrality','degree');

        console.log( i9graph.animationTime );
        i9graph.animationTimeToSlider();
        redrawAll();
    }


    function execute_animation(){
        if(!i9graph.animation_playing) return;

        i9graph.animationTime += i9graph.animationSpeed;
        timeChanged();
    }
    ...
    i9graph.animationSpeedFromSlider = function () { i9graph.animationSpeed = Math.pow(2, .1*(document.getElementById('animationspeed').value -10));}
    i9graph.animationFrameFromSlider = function () { i9graph.animationFrame = (document.getElementById('animationframe').value/1000) * (i9graph.i9g.end-i9graph.i9g.start);}

    i9graph.animationTimeToSlider = function () { document.getElementById('animationtime').value = (i9graph.animationTime - i9graph.i9g.start)/(i9graph.i9g.end-i9graph.i9g.start)*1000 }
    i9graph.animationFrameToSlider = function () { document.getElementById('animationframe').value = (i9graph.animationFrame)/(i9graph.i9g.end-i9graph.i9g.start)*1000 }
    i9graph.animationSpeedToSlider = function () { document.getElementById('animationspeed').value = 10 + 10 * Math.log2(i9graph.animationSpeed); }

    i9graph.resetAnimationOptions =function(){
        i9graph.animationTime = i9graph.i9g.start;
        i9graph.animationSpeed = 1;
        i9graph.animationFrame = 0;
        i9graph.animationTimeToSlider();
        i9graph.animationSpeedToSlider();
        i9graph.animationFrameToSlider();
        timeChanged();
    }
    ...
    var internGraph = new (function () {
        ...
        this.setNode = function (n) {
            ...
            G.vertices[4 * n.id + 2] = n.visible ? actual_size : 0;
            ...
            if(n.visible)  request_text_for_node(n.id);
        ...
        this.setEdge = function (id, e) {
            ...
            G.neighbouringEdges[e.src].push(id);
            if (e.src != e.dst) {
                G.neighbouringEdges[e.dst].push(id);
                G.neighbouringNodes[e.src].push(e.dst);  // only other neighbouring nodes than self are interresting
                G.neighbouringNodes[e.dst].push(e.src);
            }
        };
        ...
        this.setGraph = function (g) {
            ...
            g.edges.forEach(function (e, i) { G.setEdge(i, e); });
        };
    ...
    function updateInternNodesIfChanged() {
        ...
        if(i9graph.i9g && i9graph.i9g.edges) i9graph.i9g.edges.forEach(function(e,i){internGraph.setEdge(i,e); });
    }
    ...
    i9graph.setColorMeasure('centrality.age');
    ...
    function update_node_properties() {
        ...
        i9graph.i9g.nodes.forEach(function (n) {
            n.radius = i9graph.getNodeSize(n) * .05 + .01;
            n.sort_key = i9graph.getNodeSort(n);
            n.color = i9graph.hsv_to_rgb(240 * (1 - i9graph.getNodeColor(n)), .9, .9);
    ...
    i9graph.setupDropdowns = function () {
        ...
        if(lo.options){
            ...
            str += "<h3>Options:</h3>";//<form>";
            for(var o in lo.options){
                str += "<a class='inline'><input type='text' style='width:50px'"
                +" value='"+lo.options[o]+"'"
...
//            str += "</form>";
            str +="<button type='button' class='btn button-default' onclick='"
            +" var lo = i9graph.layout[i9graph.current_layout];"
