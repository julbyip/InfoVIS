...
(function (i9graph, undefined) {
    ...
    i9graph.setCentrality = function (type) { 
        i9graph.computeMeasure('centrality', type);  
        i9graph.setColorMeasure('centrality.'+type);
        i9graph.setSizeMeasure('centrality.'+type);
        i9graph.setSortMeasure('centrality.'+type);
    }
    ...
    i9graph.computeMeasure = function (type, fnc) {//type(centrality,community), fnc(degree etc..
        ...
        if (i9graph.computedMeasures[type + '.' + fnc] === undefined) {
            if(i9graph[type]===undefined) console.log('i9graph.'+type+' is not defined');
                ...
                i9graph.computedMeasures[type + "." + fnc] = {};
    ...
    i9graph.setInspectorForSelectedNode = function( n ){
        ...
        if(!n){
            ...
            for(var m in n.measure){
                ...
                var min = i9graph.computedMeasures[m].min;
                var max = i9graph.computedMeasures[m].max;

                var val = min + (max-min) * n.measure[m];

                var color = i9graph.hsv_to_rgb(240 * (1-n.measure[m]),.9,.9);
                
                color = '#'+ ((Math.floor( color[0]*256 )) . toString(16)) +
                             ((Math.floor( color[1]*256 )) . toString(16)) +
                             ((Math.floor( color[2]*256 )) . toString(16)) ;
    ...
    i9graph.setupDropdowns = function () {
        ...
        for(var c in i9graph.computedMeasures){
            ...
            var info = "["+i9graph.computedMeasures[c].min+","+i9graph.computedMeasures[c].max+"]";
            
            var html = "<a class='inline'"
                +" href=\"#\""
                +" data-toggle='tooltip' title='set Color by "+c+" "+info+"'"
                +" style='color:#"+(i9graph.color_measure==c?'991111':'111111')+"'"
                +" onclick='i9graph.setColorMeasure(\""+c+"\");' >"+a[1]+"</a>"
                +"<a href='#' "
                +" data-toggle='tooltip' title='set Size by "+c+" "+info+"'"
                +" style='color:#"+(i9graph.size_measure==c?'991111':'111111')+"'"
                +"onclick='i9graph.setSizeMeasure(\""+c+"\");'> oO</a>"
                +"<a href='#' "
                +" data-toggle='tooltip' title='set Sorting by "+c+" "+info+" (only for some layouts)'"
